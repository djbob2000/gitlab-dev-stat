...
export function SortableHeader<TData>({ header, _table }: SortableHeaderProps<TData>) {
  ...
  const toggleSortingHandler = canSort ? header.column.getToggleSortingHandler() : undefined;

  const canResize = header.column.getCanResize();
  const resizeHandler = canResize ? header.getResizeHandler() : undefined;

  const isSorted = header.column.getIsSorted();
  const sortingIcon = !isSorted ? null : (
  ...
  const thClassName = `px-2 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider relative ${
  ...
  const headerContent = isPlaceholder ? null : flexRender(columnDef, headerContext);
